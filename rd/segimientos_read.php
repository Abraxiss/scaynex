...
HAVING (((rd_servicio.Id_SERG)='$Id_SERG'));
        ...
        <?php 
          $queryO="
SELECT rd_operadores.Id_SERG, Count(rd_operadores.ID_OPERA) AS CuentaDeID_OPERA, Sum(EFECTIVO + YAPE + PLIN + OTROEF) AS TOTAL
FROM rd_operadores
GROUP BY rd_operadores.Id_SERG
HAVING (((rd_operadores.Id_SERG)='$Id_SERG'));
        ...
        <?php 
          $queryC="
SELECT rd_combustible.Id_SERG, Count(rd_combustible.ID_COMB) AS CuentaDeID_COMB, Sum(rd_combustible.COM_SOLES) AS SumaDeCOM_SOLES, Sum(rd_combustible.COM_GALONES) AS SumaDeCOM_GALONES
FROM rd_combustible
GROUP BY rd_combustible.Id_SERG
HAVING (((rd_combustible.Id_SERG)='$Id_SERG'));

             ";
                      
          $resultC=mysqli_query($conexion, $queryC);
          $filasC=mysqli_fetch_assoc($resultC);
          $CANTCOMB = $filasC ['CuentaDeID_COMB'];
          $GALONES = $filasC ['SumaDeCOM_GALONES'];
          $SOLES= $filasC ['SumaDeCOM_SOLES'];
          $numfilasC = mysqli_num_rows($resultC);
        if ($numfilasC>0) {
            ?>
          <a href="rd_combustible_read.php?rd=<?php echo $filas ['Id_SERG'] ?>&f=<?php echo $fecha;?>" class="btn btn-success btn-sm btn-block ">
            <span class=" icon-bookmark  "></span><?php echo $CANTCOMB ?>
            | G: </span><?php echo $GALONES ?>
            | S/.<?php echo $SOLES ?>  
            </a> 
          <?php
        } else {  
          ?>
          <a href="rd_combustible_read.php?rd=<?php echo $filas ['Id_SERG'];?>&f=<?php echo $fecha;?>" class="btn btn-dark btn-sm btn-block "><span class="icon-blocked"></span></a>
          <?php  
        }
        ?>
        </td>
